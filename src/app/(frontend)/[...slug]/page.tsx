...
export async function generateStaticParams() {
	...
	return [...pages, ...posts, ...postCategories, ...portofolios].map((doc) => {
		if (doc.link) {
...
export default async function Page({ params: paramsPromise }: Args) {
	...
	} else if (slugs[0] === slugPortofolio && slugs.length === 2) {
		const doc = await portofolioLoader(slugs)
	...
	const [profileConfig, siteConfig] = await Promise.all([getProfileGlobal(), getSiteGlobal()])
...
export async function generateMetadata({ params: paramsPromise }: Args): Promise<Metadata> {
	...
	try {
		...
		let doc: Page | Post | PostCategory | Portofolio | Reusable | null = null
		...
		} else if (slug[0] === slugPortofolio && slug.length === 2) {
			doc = await portofolioLoader(slug)
		...
		console.log('generateMetadata', { slug, error })
